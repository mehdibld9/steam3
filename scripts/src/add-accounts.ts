...
async function main() {
  ...
  const usedTitles = new Set(existingTitles.map((t: any) => t.title));
  const data = [];
  const count = 80;

  for (let i = 0; i < count; i++) {
    let title: string;
    do { title = pick(titles); } while (usedTitles.has(title));
    ...
    data.push({
      ...
      games: accountGames,
      pointsCost: randInt(50, 800),
      steamUsername: `steam_${randInt(10000, 99999)}`,
      steamPassword: `pass_${randInt(100000, 999999)}`,
      isAvailable,
      likesCount: randInt(0, 250),
      claimsCount: isAvailable ? randInt(0, 50) : randInt(50, 200),
      workingVotes: randInt(0, 100),
      notWorkingVotes: randInt(0, 30),
      viewCount: randInt(10, 5000),
      unlockMethod: pick(["login", "family", "gift"]),
      status,
